...
double jeu::modifChrono(struct tm timer)
{
    time_t timed;
    time(&timed);
    chronometre=difftime(mktime(&timer),timed);
...
void jeu::aleatoireGen(niveau & niveau1)
{
  static int n=4;
  Sort sort1;
  TableauDynamique * A;
  ...
  for(int j=0;j<n;j++)
  {
    int random=rand()%(200+200)-200;
    if(A->rechercherElement(random)!=-1)
    {
      j--;
    }
    else{
      A->ajouterElement(random);
    }
  }
  TableauDynamique B=*A;
  sort1.set_tabA(B);
